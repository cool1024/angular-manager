...
@Component({
  ...
  <div class="card rounded-0">
      <div class="cardiimg-top text-center card-div-img pointer m-1 rounded-0" (click)="file.click()">
          <img src="{{(loopCard.url||loopCard.base64)||'/assets/system/file-open.svg'}}" alt="Card image cap">
      </div>
      <hr>
      <div class="input-group input-group-sm p-2">
          <span class="input-group-addon bg-white" id="sizing-addon2">链接</span>
          <input type="text" class="form-control" (keyup)="loopCard.status=false" placeholder="请输入图片跳转链接" aria-describedby="sizing-addon2">
      </div>
      ...
      <input type="file" class="d-none" #file (change)="loopCard.status=false;readFile($event.target.files[0])" />
  </div>
  `,
...
export class LoopCardComponent {
  ...
  readFile(file: Blob): void {
    ...
    reader.addEventListener('load', () => { this.loopCard.base64 = reader.result; this.loopCard.file = file; });
